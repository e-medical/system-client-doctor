...
import DefaultSecondPrescription from "../../../public/assets/Labtest.png";
import { configPrescriptionTheme } from "../../services/hospitalService.ts";

export default function DefaultSecondPrescriptionCardInner() {
    const navigate = useNavigate();
    const tabs = [{ name: "Standard", path: "/process/doctor/prescription/lab-test" }];
    ...
    useEffect(() => {
        ...
        if (themesCookie) {
            try {
                ...
                const standardTheme = themes.find((t: any) => t.theme === "Standard");
                if (standardTheme?.isActive) {
                    setAppliedIndexes([0]);
                }
            } catch (err) {
    ...
    const handleApplyAll = async (index: number) => {
        try {
            ...
            if (themesCookie) {
                const themes = JSON.parse(themesCookie);
                const standardTheme = themes.find((t: any) => t.theme === "Standard");
                currentIsActive = !!standardTheme?.isActive;
            }
            ...
            const res = await configPrescriptionTheme(newIsActive, "Standard");
                ...
                if (themesCookie) {
                    try {
                        ...
                        const updatedThemes = themes.map((t: any) =>
                            t.theme === "Standard" ? { ...t, isActive: newIsActive } : t
                        );
